...
from fancy_ray_tracer import equal, matrices, tuples
...
def test_translation():
    ...
    assert equal(transform.dot(
        p), tuples.point(2, 1, 7))
    assert equal(inv.dot(
...
def test_rotX():
    ...
    assert equal(half_quarter.dot(
        p), tuples.point(0, sqrt(2) / 2, sqrt(2) / 2))
    assert equal(full_quarter.dot(
...
def test_rotX_inv():
    ...
    assert equal(half_quarter_inv.dot(
        p), tuples.point(0, (sqrt(2) / 2), -(sqrt(2) / 2)))
